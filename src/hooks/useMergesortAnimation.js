...
import { mergesortTrace } from "../utils/mergesortTrace";
import { DEFAULT_ARRAY, REVERSED_ARRAY, SMALL_ARRAY } from "../data/mergesortArrays";

/**
 * Gère toute la logique d'animation Mergesort :
 * tableau courant, étapes, index, lecture automatique, vitesse.
 */
export function useMergesortAnimation() {
  const [currentArray, setCurrentArray] = useState(DEFAULT_ARRAY);
  const [steps, setSteps]     = useState(() => mergesortTrace(DEFAULT_ARRAY));
  ...
  useEffect(() => {
    const s = mergesortTrace(currentArray);
    setSteps(s);
    setStepIdx(0);
    setIsPlaying(false);
  }, [currentArray]);

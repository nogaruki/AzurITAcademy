...
import { BINARY_SEARCH_QUIZ_QUESTIONS } from "../../data/binarySearchQuestions";
import QuizQuestion from "../../components/quiz/QuizQuestion";
import QuizScore    from "../../components/quiz/QuizScore";
import { useLang }  from "../../i18n/LangContext";

export default function BinarySearchQuizTab() {
  const [quizAnswers, setQuizAnswers] = useState({});
  const { t, lang } = useLang();
  const questions = BINARY_SEARCH_QUIZ_QUESTIONS[lang];
  ...
  const score      = Object.keys(quizAnswers).filter((k) => quizAnswers[k] === questions[k].answer).length;
  ...
  return (
    <div className="bg-card border border-line rounded-[14px] px-7 py-7 mb-10">
      <h3 className="text-[18px] font-bold mb-6">{t("quiz.title.binarysearch")}</h3>
      {questions.map((q, qi) => (
        <QuizQuestion
          key={qi}
          question={q}
